...
public class DrawUtils extends JPanel {
    ...
    protected void paintComponent(Graphics g) {
        ...
        for (ArrayList<Integer> line : polygon) {
            drawLine(line.get(0), line.get(1), line.get(2), line.get(3), cellSize, g, Color.ORANGE);
    ...
    private void drawRect(Graphics g, ArrayList<Integer> line) {
        if (!rect.isEmpty()){
    ...
    private void drawCoordinateRectangles(int n, Graphics g){
        ...
        for (int i = -nX / 2; i < nX / 2; i++){
            Line2D line = new Line2D.Double((bounds.width / 2) + cellSize * i,0,
                    (bounds.width / 2) + cellSize * i,bounds.height);
            g2.draw(line);
        }
        for (int i = -nY / 2; i < nY / 2; i++){
            Line2D line = new Line2D.Double(0,(bounds.height / 2) + cellSize * i,bounds.width,
                    (bounds.height / 2) +cellSize * i);
            g2.draw(line);
        }
    }
    ...
    public void drawLines(ArrayList<ArrayList<Integer>> lines){
        this.lines = lines;
        countWindowSize(lines);
    }

    public void drawPolygon(ArrayList<ArrayList<Integer>> polygon){
        this.polygon = polygon;
        countWindowSize(polygon);
    }

    private void countWindowSize(ArrayList<ArrayList<Integer>> polygon) {
